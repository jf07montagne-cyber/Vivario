/* ambiance.js — FIX complet (v16)
   ...
   ✅ init() exposé + auto-init (pageshow/DOM)
   ✅ Alias VivarioAmbiance (typo fréquente)

   IMPORTANT:
   👉 Les fichiers ci-dessous DOIVENT exister à la racine du repo (comme dans ton listing) :
     - ambience.mp3
     - ambience_ocean.mp3
     - ambience_focus.mp3
     - ambience_deep.mp3
...
(() => {
  ...
  // ✅ Chemins réels de TON repo (racine)
  const TRACKS = [
  ...
  async function play() {
    ...
    try {
      ...
      // Mobile peut bloquer sans interaction: pas d’erreur visible, c’est normal.
    }
  }
  ...
  // ✅ appelée par respiration.js / sound.js
  ...
  function ensureBtn() {
    if (isBreathPage) return; // aucun bouton sur respiration
