/* Vivario v1.1 — engine.js (FINAL + moods)
   - Mood audio selon la réponse à la Q "tone"
...
(() => {
  ...
  function applyMoodFromAnswer(q, values){
    ...
    // On ne change l'ambiance QUE quand la question a role="tone"
    if (String(q.role || "") !== "tone") return;

    const v = String(values[0] || "").toLowerCase();
    // Mapping IDs -> mood
    // Adapte si tes IDs sont différents dans questions_v1_1.json
    let mood = "calm";
    if (v.includes("charge") || v.includes("fatigue")) mood = "deep";
    else if (v.includes("perdu") || v.includes("confus") || v.includes("flou")) mood = "focus";
    else if (v.includes("neutre")) mood = "focus";
    else if (v.includes("bien") || v.includes("stable")) mood = "ocean";
  ...
  function finish(){
    // Avant résultat: on met une ambiance plus "deep" pour la fin
