/* Vivario v1.1 — engine.js (FINAL + scénarios uniques par personne)
   ...
   ✅ Redirections ?v=17 (résultat + retour)
*/
(() => {
  ...
  function buildScenarioForVariant(profile, variantKey, usedStrictGlobal){
    ...
    function dedupeStrictKeep(arr){
      ...
      for (const l of arr){
        ...
        if (sig.includes("merci d'avoir pris ce temps") || sig.includes("merci d’avoir pris ce temps")){
    ...
    while (lines.length < min && closingsPool.length && guard < 60){
      ...
      if ((sig.includes("merci d'avoir pris ce temps") || sig.includes("merci d’avoir pris ce temps")) &&
          lines.some(x => {
            ...
            return s.includes("merci d'avoir pris ce temps") || s.includes("merci d’avoir pris ce temps");
          })) {
        continue;
      }
  ...
  function finish(){
    ...
    // ✅ IMPORTANT: v=17
    window.location.href = "resultat.html?v=17";
  }
    ...
    try{
      ...
      if (btnQuit) {
        btnQuit.addEventListener("click", () => {
          // ✅ IMPORTANT: v=17
          window.location.href = "accueil.html?v=17";
        });
      }
