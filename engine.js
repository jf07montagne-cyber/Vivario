/* Vivario v1.1 — engine.js (TEST 6)
   - TEST5 OK + FIX:
      ✅ mémoire inter-sessions (évite de répéter les mêmes phrases)
      ✅ ton adaptatif (coach doux vs neutre) via localStorage
*/
...
(() => {
  ...
  const KEY_MEM   = "vivario_used_lines_v1_1";         // JSON array of signatures
  const MEM_MAX   = 350;
  ...
  function pickManyUniqueWeighted(rng, blocks, n, usedStrict, usedLoose){
    ...
    const tries = Math.min(140, blocks.length * 7);
  ...
  function buildScenarioForVariant(profile, variantKey){
    ...
    // ✅ mémoire: on pré-remplit usedLoose avec historique
    const mem = loadMemory();
    mem.forEach(s => usedLoose.add(String(s)));
    ...
    if (!lowEnergy) blocks.push(...pickManyUniqueWeighted(rng, B(themePackAll), 1, usedStrict, usedLoose));
    ...
    if (variantKey === "norm") blocks.push(...pickManyUniqueWeighted(rng, B(normalisation), 1, usedStrict, usedLoose));
    ...
    // ✅ on stocke en mémoire les signatures “loose” des lignes retenues
