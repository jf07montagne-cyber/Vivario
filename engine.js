/* Vivario v1.1 — engine.js (TEST 12 FIX)
   ✅ garde: perf cap, micro-variations, mémoire inter-sessions + inter-onglets, coach doux/neutre, contradictions
   ✅ FIX UI: rend les options en .option (match style.css) + sélection visuelle stable
*/
...
(() => {
  ...
  function renderQuestion(qid){
    ...
    if (elHint) {
      ...
      const min = (typeof c.min === "number") ? c.min : (multi ? 1 : 1);
      const max = (typeof c.max === "number") ? c.max : null;

      elHint.style.display = multi ? "block" : "none";
      elHint.textContent = multi
        ? (max ? `✅ Tu peux cocher ${min} à ${max} réponses.` : `✅ Tu peux cocher plusieurs réponses.`)
        : "";
    }
    ...
    opts.forEach(opt => {
      ...
      // click anywhere toggles input (label already does, but we keep stable selection class)
      row.addEventListener("change", () => {
        if (!multi) {
          // si radio: nettoyer les autres
  ...
  // -------------------------------
  // Memory inter-sessions (inchangé)
  // -------------------------------
  function loadMemory(){
    try{
  ...
  function finish(){
    ...
    window.location.href = "resultat.html?v=1200";
  }
