/* Vivario v1.1 — engine.js (FINAL + scénarios uniques par personne)
   ...
   ✅ DEDUPE + AGRESSIF: ignore ponctuation => stop répétitions “…” / “.”
*/
(() => {
  ...
  function normalizeLine(s){
    return String(s || "")
      .toLowerCase()
      .replace(/[’]/g, "'")
      .replace(/[.,!?…:;()"]/g, "")     // ✅ retire ponctuation => stop répétitions “différentes”
      .replace(/\s+/g, " ")
      .trim();
  }
  ...
  function pickManyUniqueWeighted(rng, blocks, n, usedStrict){
    ...
    const tries = Math.min(200, local.length * 8);
  ...
  function buildScenarioForVariant(profile, variantKey, usedStrictGlobal){
    ...
    function dedupeStrictKeep(arr){
      ...
      for (const l of arr){
        ...
        if (sig.includes("merci davoir pris ce temps")){
    ...
    let guard = 0;
    while (lines.length < min && closingsPool.length && guard < 60){
      guard++;
      ...
      if (sig.includes("merci davoir pris ce temps") &&
          lines.some(x => normalizeLine(x).includes("merci davoir pris ce temps"))) {
        continue;
      }
