...
(() => {
  ...
  function renderQuestion(qid){
    ...
    // Si tu as un bloc #qText (certains templates l'ont), on l'alimente aussi
    if (elText) elText.textContent = q.text || q.question || "";

    // Subtitle prioritaire si présent
  ...
  function finish(){
    ...
    window.location.href = "resultat.html?v=15"; // ✅ cohérent
  }
  ...
  async function init(){
    try{
      ...
      // ✅ Quitter => retour accueil (capture)
      if (btnQuit) {
