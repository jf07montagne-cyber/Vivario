/* Vivario v1.1 — engine.js (FINAL + scénarios uniques par personne) — PATCH v16.2
   ✅ vivario_user_id persistant (unique device)
   ✅ Seed inclut user_id => mêmes réponses ≠ même scénario sur 2 appareils
   ✅ Coach doux par défaut ON si pas encore défini
   ✅ FIX anti-répétitions visibles (doublons / coach doux / stop / merci)
   ✅ Normalize plus strict (ponctuation, ellipsis, guillemets)
*/
(() => {
  ...
  const KEY_MEM   = "vivario_used_lines_v1_1";
  const KEY_USER  = "vivario_user_id";
  const MEM_MAX   = 350;

  const CANDIDATE_CAP = 260;

  const elTitle   = document.getElementById("qTitle");
  const elText    = document.getElementById("qText"); // optionnel
  ...
  function renderQuestion(qid){
    ...
    if (elText)  elText.textContent  = q.text || q.question || "";
  ...
  function buildProfile(){
    ...
    return { userId: USER_ID, root, tone, themes, focus, vecu, besoin, posture, energie, sortie, coach, tags };
  }
  ...
  // ✅ Normalize plus strict (pour éviter doublons "..." / "." / guillemets / ponctuation finale)
  function normalizeLine(s){
    return String(s || "")
      ...
      .replace(/…/g, "...")
      .replace(/\s+/g, " ")
      .replace(/\s*([.!?]+)\s*$/g, "") // enlève ponctuation finale
      .replace(/["']/g, "")           // enlève quotes
      .trim();
  }
  ...
  function pickManyUniqueWeighted(rng, blocks, n, usedStrict){
    const out = [];
    if (!blocks.length || n <= 0) return out;

    const local = capCandidates(blocks, rng);
    const tries = Math.min(220, local.length * 9);

    for (let i=0; i<tries && out.length < n; i++){
      const b = weightedPick(rng, local);
      if (!b) continue;
      const sig = normalizeLine(b.text);
      if (!sig) continue;
      if (usedStrict.has(sig)) continue;
      usedStrict.add(sig);
      out.push(b);
    }
    return out;
  ...
  function buildScenarioForVariant(profile, variantKey, usedStrictGlobal){
    if (!SCEN) {
      return { text: "Merci d’avoir pris ce temps.\n\nTu peux avancer au rythme du jour.", usedSigs: [] };
    }
    ...
    const seed = hashString(`${seedBase}:${profile.userId}:${variantKey}:${profile.root}:${profile.tone}:${profile.energie}:${profile.sortie}:${profile.coach}`);
    const rng = mulberry32(seed);

    const ctx = buildCtx(profile);

    const usedStrict = new Set();
    loadMemory().forEach(s => usedStrict.add(String(s)));
    usedStrictGlobal.forEach(s => usedStrict.add(String(s)));
    ...
    const blocks = [];
    blocks.push(...pickManyUniqueWeighted(rng, B(rootPack.intro || []), 1, usedStrict));
    blocks.push(...pickManyUniqueWeighted(rng, B(toneOpen), 1, usedStrict));

    if (variantKey === "main" || variantKey === "calm") {
      blocks.push(...pickManyUniqueWeighted(rng, B(bridges), 1, usedStrict));
    }

    const focusIds = Array.isArray(profile.focus) ? profile.focus : [];
    const focusLines = flattenByIds(modules.themes || {}, focusIds);
    blocks.push(...pickManyUniqueWeighted(rng, B(focusLines.length ? focusLines : themePackAll), 1, usedStrict));

    if (profile.energie !== "faible") blocks.push(...pickManyUniqueWeighted(rng, B(vecuPackAll), 1, usedStrict));
    if (profile.energie !== "faible") blocks.push(...pickManyUniqueWeighted(rng, B(posturePackAll), 1, usedStrict));

    if (variantKey === "norm") blocks.push(...pickManyUniqueWeighted(rng, B(normalisation), 1, usedStrict));

    blocks.push(...pickManyUniqueWeighted(rng, B(needPackAll), 1, usedStrict));
    blocks.push(...pickManyUniqueWeighted(rng, B(variantPacks), (profile.energie === "faible") ? 1 : 2, usedStrict));
    blocks.push(...pickManyUniqueWeighted(rng, B(energyPack), 1, usedStrict));

    blocks.push(...pickManyUniqueWeighted(rng, B(variantClosings), 1, usedStrict));
    blocks.push(...pickManyUniqueWeighted(rng, B(closings), 1, usedStrict));
    ...
    // ✅ Dédoublonnage strict + règles anti répétitions "visibles"
    function dedupeStrictKeep(arr){
      const out = [];
      const seen = new Set();

      let seenMerci = false;
      let seenStopLine = false;
      let seenCoachPrefix = false;

      for (const l of arr){
        const cleaned = String(l || "").trim();
        if (!cleaned) continue;

        const sig = normalizeLine(cleaned);
        if (!sig) continue;

        if (seen.has(sig)) continue;

        // Anti "Coach doux :" spam
        if (sig.startsWith("coach doux")){
          if (seenCoachPrefix) continue;
          seenCoachPrefix = true;
        }

        // Anti "Tu peux t'arrêter ici..." répété
        if (sig.includes("tu peux t arreter ici") || sig.includes("tu peux tarreter ici") || sig.includes("tu peux t’arreter ici")){
          if (seenStopLine) continue;
          seenStopLine = true;
        }

        // Anti double "Merci d'avoir pris ce temps"
        if (sig.includes("merci davoir pris ce temps") || sig.includes("merci d avoir pris ce temps") || sig.includes("merci d’avoir pris ce temps")){
          if (seenMerci) continue;
          seenMerci = true;
        }
    ...
    lines = dedupeStrictKeep(lines);

    // garde-fou: complète si trop court sans réintroduire les répétitions
    const closingsPool = B(closings);
    let guard = 0;
    while (lines.length < min && closingsPool.length && guard < 80){
      guard++;
      const extra = weightedPick(rng, closingsPool);
      if (!extra?.text) continue;

      const candidate = microVary(fillTemplate(extra.text, ctx), rng, profile.coach).trim();
      if (!candidate) continue;

      const sig = normalizeLine(candidate);
      if (!sig) continue;

      const already = lines.some(x => normalizeLine(x) === sig);
      if (already) continue;

      // règles "merci" et "stop"
      const hasMerciAlready = lines.some(x => {
        const s = normalizeLine(x);
        return s.includes("merci davoir pris ce temps") || s.includes("merci d avoir pris ce temps") || s.includes("merci d’avoir pris ce temps");
      });
      if (hasMerciAlready && (sig.includes("merci davoir pris ce temps") || sig.includes("merci d avoir pris ce temps") || sig.includes("merci d’avoir pris ce temps"))) {
        continue;
      }

      const hasStopAlready = lines.some(x => {
        const s = normalizeLine(x);
        return s.includes("tu peux t arreter ici") || s.includes("tu peux tarreter ici") || s.includes("tu peux t’arreter ici");
      });
      if (hasStopAlready && (sig.includes("tu peux t arreter ici") || sig.includes("tu peux tarreter ici") || sig.includes("tu peux t’arreter ici"))) {
        continue;
      }

      const hasCoachAlready = lines.some(x => normalizeLine(x).startsWith("coach doux"));
      if (hasCoachAlready && sig.startsWith("coach doux")) {
        continue;
      }
    ...
    const usedSigs = lines.map(normalizeLine).filter(Boolean);
    usedSigs.forEach(s => usedStrictGlobal.add(s));
    pushMemory(usedSigs);
  ...
  function buildScenarios(profile){
    ...
    const usedStrictGlobal = new Set();

    const main = buildScenarioForVariant(profile, "main", usedStrictGlobal);
    const step = buildScenarioForVariant(profile, "step", usedStrictGlobal);
    const calm = buildScenarioForVariant(profile, "calm", usedStrictGlobal);
    const norm = buildScenarioForVariant(profile, "norm", usedStrictGlobal);
  ...
  function finish(){
    ...
    window.location.href = "resultat.html?v=16";
  }
  ...
  function setupCoachSwitch(){
    ...
    // ✅ par défaut ON si jamais défini
    ...
    try{
      ...
      if (btnQuit) {
        btnQuit.addEventListener("click", () => {
          window.location.href = "accueil.html?v=16";
        });
      }
