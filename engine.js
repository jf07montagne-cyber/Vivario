/* Vivario v1.1 — engine.js (FINAL + moods)
   - Mood audio selon la réponse à la Q role="tone"
...
(() => {
  ...
  function applyMoodFromAnswer(q, values){
    ...
    // Mapping IDs -> mood
    // ⚠️ Si tes IDs sont différents, adapte ici
    let mood = "calm";
  ...
  function finish(){
    ...
    // ✅ anti-cache
    window.location.href = "resultat.html?v=5";
  }
  ...
  function goBack(){
    // ✅ si on est à la première question -> retour accueil
    if (!history.length) {
      window.location.href = "accueil.html?v=5";
      return;
    }
