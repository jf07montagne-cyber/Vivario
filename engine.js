/* Vivario v1.1 — engine.js (TEST 15 FIX)
   ...
   ✅ AJOUT: btnQuit -> retour accueil
   ✅ FIX: scénarios raccourcis + phrase répétée entre onglets
*/
...
(() => {
  ...
  const btnQuit   = document.getElementById("btnQuit");     // ✅ AJOUT (capture)
  ...
  function buildScenarioForVariant(profile, variantKey, usedLooseGlobal){
    ...
    // ✅ FIX: boucle closings sans doublons (strict)
    while (lines.length < min && closings.length) {
      ...
      if (extra?.text){
        ...
        if (!lines.some(x => normalizeLine(x) === sig)) lines.push(candidate);
      }
      if (lines.length >= min) break;
    }

    // ✅ FIX: Dédoublonnage FINAL STRICT + anti "Merci..." répété
    const out = [];
    const seen = new Set();
    ...
    for (const l of lines){
      ...
      if (sig.includes("merci d'avoir pris ce temps") && [...seen].some(s => s.includes("merci d'avoir pris ce temps"))){
        continue;
      }
