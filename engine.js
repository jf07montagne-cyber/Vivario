/* Vivario v1.1 — engine.js (FIX COMPLET)
   ...
   ✅ Mood + scénarios + mémoire
   ✅ Quitter => retour accueil
   ✅ FIX scénarios:
      - anti "Merci d’avoir pris ce temps" répété entre onglets
      - dédoublonnage STRICT (normalizeLine) au lieu de loose
      - maintien min_sentences en ajoutant des closings uniques
*/
...
(() => {
  ...
  function buildScenarioForVariant(profile, variantKey, usedStrictGlobal){
    ...
    // ✅ set de signatures strict : mémoire + global (évite répétitions inter-onglets)
    ...
    // ✅ cap max avant dédoublonnage fin
    if (lines.length > max) lines = lines.slice(0, max);

    // ✅ dédoublonnage final STRICT + anti "Merci..." répété
    function dedupeStrictKeep(arr){
      ...
      for (const l of arr){
        ...
        // anti "Merci d’avoir pris ce temps" qui revient partout
    ...
    // ✅ si on est sous min, on rajoute des closings UNIQUE (sans répéter)
    ...
    // sécurité max après remplissage min
    if (lines.length > max) lines = lines.slice(0, max);
  ...
  function finish(){
    ...
    window.location.href = "resultat.html?v=15";
  }
    ...
    try{
      ...
      if (btnQuit) {
        btnQuit.addEventListener("click", () => {
          window.location.href = "accueil.html?v=15";
        });
      }
