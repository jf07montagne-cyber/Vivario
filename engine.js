/* Vivario v1.2 — engine.js
   ✅ Scénarios beaucoup moins répétitifs
   ✅ Structure différente par onglet (main/step/calm/norm) => rendu plus "unique"
   ✅ Seed inclut user_id
   ✅ Coach doux par défaut ON si jamais défini
   ✅ Redirections ?v=18 (résultat + retour)
   ✅ DEDUPE agressif : ignore ponctuation + espaces
*/
(() => {
  const STORAGE_KEY = "vivario_session_v1_2";
  const QUESTIONS_URL = "./questions_v1_1.json";
  const SCENARIOS_URL = "./scenarios_v1_1.json";

  const KEY_COACH = "vivario_coach_soft";
  const KEY_MEM   = "vivario_used_lines_v1_2";
  ...
  function microVary(line, rng, coach){
    ...
    if (rng() < 0.08 && !/[.!?…]$/.test(s)) s += ".";
    if (rng() < 0.06 && /[.!]$/.test(s)) s = s.replace(/[.!]$/, "…");
  ...
  function capCandidates(blocks, rng){
    ...
    const out = [];
    const used = new Set();
    const tries = Math.min(900, blocks.length * 2);
  ...
  function pickManyUniqueWeighted(rng, blocks, n, usedStrict){
    ...
    const tries = Math.min(260, local.length * 10);
  ...
  function dedupeStrictKeep(lines){
    const out = [];
    const seen = new Set();
  ...
  function buildScenarioForVariant(profile, variantKey, usedStrictGlobal){
    if (!SCEN) {
      return { text: "Tu peux avancer au rythme du jour.\n\nSi tu veux, reviens quand ce sera plus simple.", usedSigs: [] };
    }
    ...
    const max = vMeta.max_sentences ?? meta.max_sentences ?? 11;

    // seed variant + user + profil
    ...
    const themePickPool = B(focusLines.length ? focusLines : themePackAll);

    // --- STRUCTURE PAR VARIANTE (IMPORTANT) ---
    const blocks = [];

    // 1) une intro racine + une phrase de ton
    ...
    // 2) structure spécifique
    if (variantKey === "step") {
      // Step = plus concret, plus "checklist"
      blocks.push(...pickManyUniqueWeighted(rng, B(themePickPool), 1, usedStrict));
      blocks.push(...pickManyUniqueWeighted(rng, B(needPackAll), 1, usedStrict));
      // 2 à 3 micro-steps (très distinct)
      blocks.push(...pickManyUniqueWeighted(rng, B(variantPacks), (profile.energie === "faible") ? 2 : 3, usedStrict));
    ...
    else if (variantKey === "calm") {
      // Calm = pont + corps + respiration
      blocks.push(...pickManyUniqueWeighted(rng, B(bridges), 1, usedStrict));
      blocks.push(...pickManyUniqueWeighted(rng, B(variantPacks), 2, usedStrict));
      blocks.push(...pickManyUniqueWeighted(rng, B(energyPack), 1, usedStrict));
      blocks.push(...pickManyUniqueWeighted(rng, B(variantClosings), 1, usedStrict));
    }
    else if (variantKey === "norm") {
      // Norm = psychoéducation + normalisation en priorité
      blocks.push(...pickManyUniqueWeighted(rng, B(normalisation), 2, usedStrict));
      blocks.push(...pickManyUniqueWeighted(rng, B(themePickPool), 1, usedStrict));
      blocks.push(...pickManyUniqueWeighted(rng, B(variantPacks), 1, usedStrict));
      blocks.push(...pickManyUniqueWeighted(rng, B(variantClosings), 1, usedStrict));
    }
    else {
      // main = récit + repères
    ...
    // 3) toujours 1 “closing” global, MAIS unique entre variantes
    ...
    // si trop court : on complète avec closings/bridges/energy (mais sans répéter)
    const fillerPools = [
      ...B(bridges),
      ...B(energyPack),
      ...B(closings)
    ...
    let guard = 0;
    while (lines.length < min && fillerPools.length && guard < 120){
      guard++;
      const extra = weightedPick(rng, fillerPools);
      ...
      if (!sig) continue;
      if (lines.some(x => normalizeLine(x) === sig)) continue;
      if (usedStrict.has(sig)) continue;
      usedStrict.add(sig);
  ...
  function buildFinalMessage(profile){
    // (tu peux garder ton buildFinalMessage actuel si tu veux — ici on reste simple)
    const tone = labelTone(profile.tone);
    const theme1 = profile.focus?.[0] ? labelTheme(profile.focus[0]) : "ce que tu traverses";
    const need1 = profile.besoin?.[0] ? labelNeed(profile.besoin[0]) : "douceur";
    return `Tu es ${tone} aujourd’hui.\n\nTon attention se tourne vers ${theme1}.\n\nTon besoin principal ressemble à : ${need1}.\n\nAvance doucement : une seule chose utile suffit.`;
  }
  ...
  function finish(){
    ...
    window.location.href = "resultat.html?v=18";
  }
    ...
    try{
      ...
      if (btnQuit) {
        btnQuit.addEventListener("click", () => {
          window.location.href = "accueil.html?v=18";
        });
      }
