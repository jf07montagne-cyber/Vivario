/* Vivario v1.1 — engine.js (TEST 5)
   - Garde tout ce qui marche (questionnaire + session localStorage + mood audio + 4 onglets)
   - Ajoute:
      ✅ Blocs combinés (objets {text, requires, forbids, weight, tags})
      ✅ Règles de contradictions (filtrage contextuel)
      ✅ Pondération (weight)
      ✅ Anti-répétition renforcé (dédupe strict + similar)
*/
...
(() => {
  ...
  // -------------------------------
  // Profile IDs (inchangé + focus smart)
  // -------------------------------
  ...
  function filterBlocks(profile, blocks){
    // contradictions globales simples
    // - si sortie stop/pas_aide : éviter trop "plan d’action"
    // - si energie faible : éviter "liste de tâches"
    // - si besoin presence : éviter injonctions fortes
    return blocks.filter(b => {
      ...
      if (isExit && (t.includes("un pas concret") || t.includes("fais") || t.includes("dois") || t.includes("objectif"))) {
        // on garde quand même si le bloc est explicitement doux via tags
      ...
      if (wantPresence && (t.includes("choisis une action") || t.includes("fais") || t.includes("définis"))) {
        if (!b.tags.includes("soft")) return false;
      }
      return true;
    });
  ...
  function pickManyUniqueWeighted(rng, blocks, n, usedStrict, usedLoose){
    ...
    const tries = Math.min(120, blocks.length * 6);
  ...
  function buildScenarioForVariant(profile, variantKey){
    ...
    const seed = hashString(`${seedBase}:${variantKey}:${profile.root}:${profile.tone}:${profile.energie}:${profile.sortie}`);
    ...
    // Convert to blocks + apply contradictions
    const B = (arr) => filterBlocks(profile, asBlocks(arr));

    const blocks = [];

    // 1) root intro (1)
    blocks.push(...pickManyUniqueWeighted(rng, B(rootPack.intro || []), 1, usedStrict, usedLoose));

    // 2) tone opening (1)
    blocks.push(...pickManyUniqueWeighted(rng, B(toneOpen), 1, usedStrict, usedLoose));

    // 3) bridge (0-1)
    if (variantKey === "main" || variantKey === "calm") {
      blocks.push(...pickManyUniqueWeighted(rng, B(bridges), 1, usedStrict, usedLoose));
    }

    // 4) themes: focus first (1), then optional second
    const focusIds = Array.isArray(profile.focus) ? profile.focus : [];
    const focusLines = flattenByIds(modules.themes || {}, focusIds);

    const lowEnergy = profile.energie === "faible";
    blocks.push(...pickManyUniqueWeighted(rng, B(focusLines.length ? focusLines : themePackAll), 1, usedStrict, usedLoose));
    if (!lowEnergy) {
      blocks.push(...pickManyUniqueWeighted(rng, B(themePackAll), 1, usedStrict, usedLoose));
    }

    // 5) vecu (0-1)
    if (!lowEnergy) blocks.push(...pickManyUniqueWeighted(rng, B(vecuPackAll), 1, usedStrict, usedLoose));

    // 6) posture (0-1)
    if (!lowEnergy) blocks.push(...pickManyUniqueWeighted(rng, B(posturePackAll), 1, usedStrict, usedLoose));

    // 7) need (1) + (norm adds 1 extra normalisation)
    if (variantKey === "norm") {
      blocks.push(...pickManyUniqueWeighted(rng, B(normalisation), 1, usedStrict, usedLoose));
    }
    blocks.push(...pickManyUniqueWeighted(rng, B(needPackAll), 1, usedStrict, usedLoose));

    // 8) variant pack (2 main/step, 2 calm, 2 norm) but filtered by contradictions
    blocks.push(...pickManyUniqueWeighted(rng, B(variantPacks), lowEnergy ? 1 : 2, usedStrict, usedLoose));

    // 9) energy (0-1)
    blocks.push(...pickManyUniqueWeighted(rng, B(energyPack), 1, usedStrict, usedLoose));

    // 10) variant closing (1) + closing (1)
    blocks.push(...pickManyUniqueWeighted(rng, B(variantClosings), 1, usedStrict, usedLoose));
    blocks.push(...pickManyUniqueWeighted(rng, B(closings), 1, usedStrict, usedLoose));

    // render lines (template)
    let lines = blocks.map(b => fillTemplate(b.text, ctx).trim()).filter(Boolean);

    // last anti-similar sweep
  ...
  function buildScenarios(profile){
    ...
    return [
      { key: "main", title: rootTitle, text: buildScenarioForVariant(profile, "main") },
      { key: "step", title: "Un pas concret", text: buildScenarioForVariant(profile, "step") },
      { key: "calm", title: "Apaisement", text: buildScenarioForVariant(profile, "calm") },
      { key: "norm", title: "Normalisation", text: buildScenarioForVariant(profile, "norm") }
    ];
  }
