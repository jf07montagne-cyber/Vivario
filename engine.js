/* Vivario v1.1 — engine.js (TEST 13)
   ✅ audio mood inchangé
   ✅ UI options inchangé
   ✅ FIX scenarios: +longs, +variés, anti-doublon strict, "pas concret" toujours action
*/
...
(() => {
  ...
  function renderCounter(){
    ...
    elCounter.textContent = (idx >= 0 && total) ? `Question ${idx + 1} / ${total}` : "Questionnaire v1.1";
  }
  ...
  function renderQuestion(qid){
    ...
    opts.forEach(opt => {
      ...
      row.addEventListener("change", () => {
        if (!multi) {
          const all = [...elOptions.querySelectorAll("input[type=radio]")];
          all.forEach(i => {
            const r = i.closest(".option");
            if (r) r.classList.toggle("is-selected", i.checked);
          });
        } else {
  ...
  function ensureActionLine(profile, rng){
    const actions = [
      "Choisis une micro-action de 5 minutes : écrire 3 lignes sur ce qui te pèse, puis t’arrêter.",
      "Prends un papier : note 1 chose certaine + 1 chose incertaine. Rien de plus.",
      "Fais un petit tri : ce que tu contrôles / ce que tu ne contrôles pas (2 colonnes).",
      "Envoie un message simple à toi-même : “Aujourd’hui, je fais juste une chose.”",
      "Si c’est possible : range un petit espace (2 minutes). Ça remet un peu d’ordre dedans."
    ];

    // adapte légèrement au thème
    if ((profile.themes || []).includes("finances")) {
      actions.push("Finances : écris juste 3 chiffres (revenus, charges, reste) — sans décider tout de suite.");
    }
    if ((profile.themes || []).includes("couple")) {
      actions.push("Couple : prépare 1 phrase neutre à dire (“j’ai besoin de 10 minutes pour clarifier”).");
    }
  ...
  function buildScenarioForVariant(profile, variantKey, usedLooseGlobal){
    ...
    // ✅ plus long par défaut
    const meta = SCEN.meta || {};
    const vMeta = (meta.variants && meta.variants[variantKey]) ? meta.variants[variantKey] : {};
    const min = vMeta.min_sentences ?? meta.min_sentences ?? 9;
    const max = vMeta.max_sentences ?? meta.max_sentences ?? 13;
    ...
    // ✅ structure différente selon l’onglet
    ...
    blocks.push(...pickManyUniqueWeighted(rng, B(focusLines.length ? focusLines : themePackAll), 2, usedLoose));
    ...
    if (variantKey === "norm") blocks.push(...pickManyUniqueWeighted(rng, B(normalisation), 2, usedLoose));

    blocks.push(...pickManyUniqueWeighted(rng, B(needPackAll), 2, usedLoose));
    blocks.push(...pickManyUniqueWeighted(rng, B(variantPacks), (profile.energie === "faible") ? 1 : 3, usedLoose));
    blocks.push(...pickManyUniqueWeighted(rng, B(energyPack), 1, usedLoose));

    // ✅ “pas concret” : ajoute une action garantie
    if (variantKey === "step") {
      const action = ensureActionLine(profile, rng);
      blocks.push({ text: action, weight: 2 });
    }

    blocks.push(...pickManyUniqueWeighted(rng, B(variantClosings), 1, usedLoose));
    blocks.push(...pickManyUniqueWeighted(rng, B(closings), 2, usedLoose));
    ...
    // ✅ anti-doublon STRICT (avant tu avais des doublons visibles)
    const out = [];
    const seen = new Set();
  ...
  function finish(){
    ...
    window.location.href = "resultat.html?v=1300";
  }
