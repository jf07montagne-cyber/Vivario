/* Vivario v1.1 — engine.js (TEST 10 FIX UI)
   ✅ Fix UI options alignées (classe .option + .label + .is-selected)
   ✅ Garde: perf anti-freeze + micro-variations + mémoire + coach + scénarios (validé)
*/
...
(() => {
  ...
  // ✅ UI helper: applique .is-selected sur les cartes
  function syncSelectedClasses(q){
    if (!elOptions) return;
    const multi = isMulti(q);
    const rows = [...elOptions.querySelectorAll("label.option")];

    if (!rows.length) return;

    if (!multi) {
      // radio: une seule sélection visuelle
      rows.forEach(r => {
        const input = r.querySelector("input");
        r.classList.toggle("is-selected", !!input?.checked);
      });
    } else {
      // checkbox: plusieurs possibles
      rows.forEach(r => {
        const input = r.querySelector("input");
        r.classList.toggle("is-selected", !!input?.checked);
      });
  ...
  function renderQuestion(qid){
    ...
    opts.forEach(opt => {
      // ✅ IMPORTANT: class "option" (pas "opt") pour matcher style.css
      ...
      row.innerHTML = `
        <input type="${type}" name="${name}" value="${opt.id}" ${checked ? "checked" : ""} aria-label="${opt.label}">
        <span class="label">${opt.label}</span>
      `;

      // ✅ Click/Change: maintien du style sélectionné
      row.addEventListener("click", (e) => {
        // Laisse le navigateur gérer l'input, puis sync
        // micro-delay pour être sûr que checked est à jour
        requestAnimationFrame(() => syncSelectedClasses(q));
      });

      elOptions.appendChild(row);
    });

    // ✅ applique l'état visuel si réponses déjà sauvegardées
    syncSelectedClasses(q);
    renderCounter();
  }
  ...
  function buildScenarioForVariant(profile, variantKey, usedLooseGlobal){
    ...
    let lines = blocks.map(b => microVary(fillTemplate(b.text, ctx).trim(), rng, profile.coach)).filter(Boolean);
