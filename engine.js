/* Vivario v1.1 — engine.js (TEST 10 FIX FULL)
   ✅ Fix UI options (alignement + sélection visible) : .option + .is-selected
   ✅ Fix perf (anti-freeze mobile)
   ✅ Micro-variations (bonus)
   ✅ Mémoire session + par onglet (validé)
   ✅ Coach doux / neutre (validé)
   ✅ Mood audio via Q1 (role:tone) (validé)
*/
...
(() => {
  ...
  // Perf guard: limite de candidats par module (évite freeze quand la bibliothèque grossit)
  ...
  // ✅ FIX UI: .option + .label + is-selected + listeners de refresh
  function renderQuestion(qid){
    ...
    function refreshSelectedUI(){
      ...
      rows.forEach(r => {
        ...
        r.classList.toggle("is-selected", !!input?.checked);
      });
    }

    // au rendu + au change
    refreshSelectedUI();
    elOptions.querySelectorAll("input").forEach(inp => {
      inp.addEventListener("change", refreshSelectedUI, { passive: true });
    });
  ...
  function pickManyUniqueWeighted(rng, blocks, n, usedLoose){
    ...
    const tries = Math.min(120, local.length * 6);
  ...
  // -------------------------------
  // Scenario generation (mémoire inter-onglets + micro variations)
  // -------------------------------
  function buildScenarioForVariant(profile, variantKey, usedLooseGlobal){
    ...
    // ✅ usedLoose combine: inter-sessions + inter-onglets (global)
    ...
    // Final dedupe (anti répétition interne)
    const out = [];
    const seen = new Set();
    ...
    // ✅ Update global set (anti-répétition inter-onglets)
    usedSigs.forEach(s => usedLooseGlobal.add(s));

    // ✅ Persist inter-sessions
    pushMemory(usedSigs);
  ...
  function buildScenarios(profile){
    ...
    const usedLooseGlobal = new Set(); // ✅ inter-onglets
  ...
  function finish(){
    ...
    // Draft pour résultat (coach switch)
    session.draft = {
      coach: profile.coach,
      profile,
      scenarios,
  ...
  async function init(){
    try{
      ...
      if (btnNext) btnNext.addEventListener("click", goNext, { passive: true });
      if (btnBack) btnBack.addEventListener("click", goBack, { passive: true });
