...
(() => {
  ...
  function buildFinalMessage(profile){
    // On reste sobre + vrai + “ça te ressemble” (uniquement via IDs)
    const TONE = {
    ...
    // focus thèmes = 0..2 (on ne “déduit” rien : on reprend juste les IDs)
    const themeLabel = (id) => ({
    ...
    // besoin : on prend le 1er (fiable, car sélection multi ordonnée)
    const firstNeed = (profile.besoin && profile.besoin[0]) ? profile.besoin[0] : "indetermine";

    // phrase finale = 4 blocs courts, lisibles
    return [
