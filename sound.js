/* Vivario — sound.js (V6.1 FIX++)
   ...
   - Respiration NE CHANGE PAS le réglage ambiance (OFF reste OFF)
...
(() => {
  ...
  // ✅ stop uniquement ambiance (ne coupe pas le souffle)
  ...
  async function startBreathing({
    ...
    // ✅ IMPORTANT: NE PAS modifier l'état ambiance ON/OFF
    breathState = {
      inhale, exhale, hold,
      t0: performance.now(),
      affectAmbience,
    ...
    if (muteAmbienceWhileBreath) {
      ...
      // Si on veut que l’ambiance respire aussi -> il faut que l'ambiance soit ON
      if (affectAmbience && isOn()) await startAmbience(true);
    }

    // Son respiration (souffle) : on le joue même si ambiance OFF
    if (affectBreath) {
  ...
  function stopBreathing() {
    ...
    // Remet l’ambiance comme avant (uniquement si elle tournait et si ON)
