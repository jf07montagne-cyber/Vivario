...
(() => {
  ...
  function toggle() {
    ...
    setOn(next);
    if (next) {
      // si on rallume, on tente de jouer (sinon armé)
      play();
    } else {
      pause();
    }
  ...
  function updateUI() {
    ...
    const on = getOn();
    const icon = on ? "🔊" : "🔇";
    const label = on ? (isPlaying ? "Son : ON" : (armed ? "Touchez pour démarrer" : "Son : ON")) : "Son : OFF";
    btn.textContent = `${icon} ${label}`;
    btn.setAttribute("aria-pressed", on ? "true" : "false");
  }

  // Démarrage fiable : on tente play, sinon on attend le premier geste
  function armOnFirstUserGesture() {
    ...
    const handler = async () => {
      if (!getOn()) return;
      // au premier geste, on essaye réellement de jouer
      await play();
      armed = false;
      updateUI();
  ...
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "visible") {
      ...
      // on peut laisser tourner, mais certains navigateurs coupent
      // on ne force pas pause pour éviter la sensation de “coupure”
    }
  });
