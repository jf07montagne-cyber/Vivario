/* =========================================================
   ...
   - FIX débordement (coins arrondis Android) : auto-fit + canvas inset CSS
   - bronchioles plus fines (niveau 3, procedural léger, Android-safe)
   - veines/texture procédurales (cheap)
   - respiration plus "soft" (shader easing + gonflement réduit)
...
(() => {
  ...
  /* =========================================================
     ✅ WEBGL (Android-safe) + auto-fit + bronchioles/veines
  ========================================================= */
  ...
  // --- precision: force match in VS/FS (Android)
  ...
  // ===== Shader (soft + veines procédurales) =====
  const VS = `
    ${PREC}
    attribute vec3 aPos;
    attribute vec3 aNor;
    ...
    void main(){
      ...
      float b = softEase(uBreath); // ✅ respiration plus "soft"
      vTube = uIsTube;
      vInner = uInner;

      if (uIsTube < 0.5) {
        // ✅ gonflement légèrement réduit (évite débordement + plus doux)
        float swell = 0.08 + 0.28*b;

        p.x *= 0.95;
        p.y *= 1.20;
        p.z *= 0.88;

        float lobe = 0.055 * sin(p.y*5.0 + uTime*0.85 + uSide*0.8);
        float rib  = 0.045 * sin(p.x*6.3 + p.y*2.2 + uTime*0.55);
        float front= 0.038 * sin(p.z*6.0 + p.y*1.8 - uTime*0.45);
        ...
        vSss = (0.24 + 0.56*b) + (uInner > 0.5 ? 0.18 : 0.0);
      } else {
        float pulse = 0.010 * sin(uTime*2.0) + 0.020*b;
        p *= (1.0 + pulse);
        vSss = 0.12 + 0.22*b;
      }
  ...
  const FS = `
    ...
    void main(){
      ...
      float b = softEase(uBreath);

      vec3 baseLung  = mix(vec3(0.07,0.16,0.30), vec3(0.14,0.32,0.62), 0.42 + 0.58*b);
      vec3 baseInner = mix(vec3(0.10,0.22,0.40), vec3(0.18,0.40,0.72), 0.52 + 0.48*b);
      vec3 baseTube  = mix(vec3(0.56,0.84,1.00), vec3(0.78,0.96,1.00), 0.35 + 0.65*b);
      ...
      float spec = pow(max(dot(N,H),0.0), 52.0) * (0.12 + 0.30*b);

      float sss = (0.14 + 0.34*vSss) * (0.72 - ndl);
      vec3 sssCol = vec3(0.22,0.52,0.90) * sss;

      vec3 col = base * (0.22 + 0.95*ndl) + sssCol + vec3(spec) + rim*vec3(0.20,0.52,0.95);

      // ✅ Veines procédurales (cheap) uniquement sur poumons (pas tubes)
      if (vTube < 0.5) {
        // pattern stable (sin/cos), pas de texture, Android-safe
        float a = sin(vW.x*7.0 + sin(vW.y*6.0));
        float b2 = sin(vW.y*8.0 + sin(vW.z*7.0));
        float c = sin(vW.z*6.5 + sin(vW.x*7.5));
        float v = (a*b2 + b2*c + a*c) * 0.5;

        float line = smoothstep(0.78, 0.92, abs(v));
        float depth = 0.10 + 0.10*b; // un peu plus visible en inhale
        col = mix(col, col*vec3(0.82,0.88,0.95), line*depth);
      }
  ...
  // Tube along polyline
  function tubeMesh(path, radius=0.06, radialSeg=14){
  ...
  // --- bronchioles niveau 3 (petit procedural, limité pour perf Android)
  function makeBranch(p0, p1, bend=0.06){
  ...
  function initGL(){
    ...
    // ✅ niveau 3 (bronchioles plus fines) : 2 petites branches par branche niveau 2
    const l3 = [];
    l3.push(makeBranch(leftUp[1],   [-0.60,  0.16, -0.04], 0.05));
    l3.push(makeBranch(leftUp[1],   [-0.56,  0.04, -0.06], 0.04));
    l3.push(makeBranch(leftDown[1], [-0.64, -0.26, -0.05], 0.04));
    l3.push(makeBranch(leftDown[1], [-0.54, -0.34, -0.06], 0.04));

    l3.push(makeBranch(rightUp[1],  [ 0.60,  0.16, -0.04], 0.05));
    l3.push(makeBranch(rightUp[1],  [ 0.56,  0.04, -0.06], 0.04));
    l3.push(makeBranch(rightDown[1],[ 0.64, -0.26, -0.05], 0.04));
    l3.push(makeBranch(rightDown[1],[ 0.54, -0.34, -0.06], 0.04));

    // Build tubes (radial segments réduits sur fins => perf)
    const t1 = tubeMesh(tr,       0.070, 16);
    const t2 = tubeMesh(leftMain, 0.054, 14);
    const t3 = tubeMesh(rightMain,0.054, 14);

    const t4 = tubeMesh(leftUp,   0.036, 12);
    const t5 = tubeMesh(leftDown, 0.034, 12);
    const t6 = tubeMesh(rightUp,  0.036, 12);
    const t7 = tubeMesh(rightDown,0.034, 12);
    ...
    // niveau 3 (plus fin)
    for (const path of l3) {
      const tt = tubeMesh(path, 0.022, 10);
      merged = mergeMeshes(merged, tt);
    }
  ...
  // ✅ auto-fit : scale dépend du canvas + réduit un peu à inhale max
  function computeFitScale(aspect, breath01){
    const a = clamp(aspect, 0.55, 1.60);
    const base = lerp(0.84, 0.92, (a - 0.55) / (1.60 - 0.55));
    const shrink = 1.0 - 0.07 * clamp(breath01, 0, 1); // inhale => un peu plus petit
    return base * shrink;
  }
  ...
  function renderGL(info, nowMs){
    ...
    const S = m4();
    const fit = computeFitScale(aspect, info.breath01);
    m4Scale(S, fit, fit, fit);

    const T  = m4(); m4Translate(T, 0, -0.03, -0.52);

    const M1 = m4(); m4Mul(M1, RY, RX);
    const M2 = m4(); m4Mul(M2, M1, S);     // R * S
    const M  = m4(); m4Mul(M, T, M2);      // T * (R*S)
  ...
  /* ---------------------------
     Particules 2D (airCanvas) — inchangé
  --------------------------- */
