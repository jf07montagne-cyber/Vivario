/* =========================================================
   Vivario — respiration.js (v26)
   WAOUH VISUAL UPGRADE (Android-safe)
   - Poumons: dual-layer + shading organique + veines procédurales
   - Bronches: niveau 3 + diamètres progressifs
   - Alvéoles: nuage bi-lobes + respiration plus soft
   - Oxygénation: variation couleur selon cycle
   (NE TOUCHE PAS au reste validé : UI/voix/rythme/boutons/canvas)
   ========================================================= */
...
(() => {
  ...
  /* =========================================================
     ✅ WEBGL (Android-safe) — VISUAL WAOUH
  ========================================================= */
  ...
  // --- precision: force same in VS+FS (fix Android mismatch)
  ...
  // ===== WAOUH Shader: organique + veines procédurales + oxygénation =====
  const VS = `
    ${PREC}
    attribute vec3 aPos;
    attribute vec3 aNor;
    ...
    uniform float uIsTube;   // 1.0 = tubes, 0.0 = poumon
    uniform float uInner;    // 1.0 = couche interne
    ...
    void main(){
      ...
      // léger "softening" de l’anim (moins chewing-gum)
      float bb = b*b*(3.0-2.0*b);

      if (uIsTube < 0.5) {
        // ====== POUMON : silhouette + lobes + plis doux ======
        // base aspect
        p.x *= 0.92;
        p.y *= 1.22;
        p.z *= 0.88;

        // inflation douce + tension (plus organique)
        float swell = 0.09 + 0.28*bb;

        // plis / lobes (sin stable)
        float lobe = 0.065 * sin(p.y*4.8 + uTime*0.70 + uSide*0.9);
        float rib  = 0.050 * sin(p.x*6.2 + p.y*2.0 + uTime*0.45);
        float front= 0.040 * sin(p.z*6.1 + p.y*1.7 - uTime*0.35);

        float bumps = 1.0 + lobe + rib + front;

        // courbure approximée (pour moduler les veines/couleur)
        vCurv = bumps;
        ...
        float heart = inner * smoothstep(-0.36, 0.32, p.y) * smoothstep(-0.15, 0.55, p.z);
        p.x += uSide * (-0.18 * heart);

        // séparation des lobes
        p.x += uSide * 0.64;

        // couche interne légèrement plus petite
        if (uInner > 0.5) p *= 0.962;

        // normale approx
        n = normalize(p);

        // SSS + inner boost
        vSss = (0.28 + 0.54*bb) + (uInner > 0.5 ? 0.16 : 0.0);
      } else {
        // ====== TUBES : pulse très subtil ======
        float pulse = 0.008 * sin(uTime*1.8) + 0.018*bb;
        p *= (1.0 + pulse);
        vCurv = 1.0;
        vSss = 0.16 + 0.22*bb;
      }
  ...
  const FS = `
    ...
    uniform float uBreath;
    uniform float uOxy;   // 0..1 (oxygénation)

    // hash/noise ultra léger (Android-safe)
    float hash3(vec3 p){
    ...
    void main(){
      ...
      // ========= palettes (oxy: plus lumineux/cyan) =========
      vec3 deox = vec3(0.06,0.13,0.28);   // plus profond
      vec3 ox   = vec3(0.16,0.44,0.82);   // plus oxygéné

      vec3 baseLung  = mix(deox, ox, 0.35 + 0.65*oxy);
      vec3 baseInner = mix(deox*1.15, ox*1.08, 0.45 + 0.55*oxy);
      vec3 baseTube  = mix(vec3(0.52,0.80,1.00), vec3(0.76,0.96,1.00), 0.30 + 0.70*oxy);
      ...
      // ========= veines procédurales (couleur seulement, stable) =========
      // coordonnées "organiques"
      vec3 p = vW * 2.2;
      float n1 = vnoise(p * 1.7);
      float n2 = vnoise(p * 3.3 + vec3(2.1,1.3,0.7));
      float veins = smoothstep(0.62, 0.92, n1*0.65 + n2*0.45);

      // veines plus visibles à l’oxygénation + sur zones de courbure
      float veinStrength = (0.06 + 0.10*oxy) * (0.75 + 0.25*rim) * (0.65 + 0.35*(vCurv-0.95));
      vec3 veinCol = vec3(0.20,0.55,0.95) * veinStrength * veins;

      // ========= lumière organique =========
      vec3 H = normalize(L + V);
      float spec = pow(max(dot(N,H),0.0), 56.0) * (0.10 + 0.28*oxy);

      // backlight (SSS simple)
      float back = max(dot(-L, N), 0.0);
      float sss = (0.10 + 0.34*vSss) * (0.45 + 0.55*back) * (0.55 + 0.45*oxy);
      vec3 sssCol = vec3(0.18,0.50,0.95) * sss;

      // base shading
      float diffuse = 0.22 + 0.92*ndl;
      vec3 col = base * diffuse + sssCol + vec3(spec) + rim*vec3(0.20,0.55,0.98) + veinCol;

      // léger “velours” (grain très fin)
      float g = (vnoise(vW*9.0 + vec3(0.0, 0.0, oxy*3.0)) - 0.5) * 0.02;
      col += g;
  ...
  function initGL(){
    ...
    // diamètres progressifs
    const t1  = tubeMesh(tr,       0.070, 18);
    const t2  = tubeMesh(leftMain, 0.052, 16);
    const t3  = tubeMesh(rightMain,0.052, 16);

    const t4  = tubeMesh(leftUp,   0.036, 14);
    const t5  = tubeMesh(leftDown, 0.034, 14);
    const t6  = tubeMesh(rightUp,  0.036, 14);
    const t7  = tubeMesh(rightDown,0.034, 14);

    const t8  = tubeMesh(lUpA,     0.022, 12);
    const t9  = tubeMesh(lUpB,     0.021, 12);
    const t10 = tubeMesh(lDnA,     0.021, 12);
    const t11 = tubeMesh(lDnB,     0.020, 12);

    const t12 = tubeMesh(rUpA,     0.022, 12);
    const t13 = tubeMesh(rUpB,     0.021, 12);
    const t14 = tubeMesh(rDnA,     0.021, 12);
    const t15 = tubeMesh(rDnB,     0.020, 12);
    ...
    merged = mergeMeshes(merged,t8);
    merged = mergeMeshes(merged,t9);
    merged = mergeMeshes(merged,t10);
    merged = mergeMeshes(merged,t11);
    merged = mergeMeshes(merged,t12);
    merged = mergeMeshes(merged,t13);
    merged = mergeMeshes(merged,t14);
    merged = mergeMeshes(merged,t15);
  ...
  // oxygénation lissée (évite clignotement)
  let oxySmooth = 0.0;
  ...
  function renderGL(info, nowMs){
    ...
    const T  = m4(); m4Translate(T, 0, -0.12, -0.30);
    ...
    // oxy: suit le volume (baisse à l’expiration)
  ...
  /* ---------------------------
     Alvéoles / particules 2D (airCanvas) — WAOUH
  --------------------------- */
  ...
  function resetAlveoli(){
    alveoli = [];
    const count = 160; // plus dense mais léger
    for(let i=0;i<count;i++){
      const side = Math.random() < 0.5 ? -1 : 1;
      // coordonnées locales "lobe"
      const u = Math.random();
      const v = Math.random();
      // distribution plus centrale (belle)
      const r = Math.sqrt(u);
      const a = v * Math.PI * 2;
  ...
  function drawAlveoli(info, nowMs){
    ...
    // rayon lobe (respire)
    const rx = lerp(0.18, 0.22, b) * w;
    const ry = lerp(0.23, 0.28, b) * h;

    // lueur globale (oxygénation)
    airCtx.globalCompositeOperation = "lighter";

    for(const p of alveoli){
      // mouvement très soft (pas de “bulles qui montent” random)
      ...
      // forme lobe : ellipse + un peu de “tissu”
      let x = x0 + Math.cos(p.a + wob) * rr * rx;
      let y = cy + Math.sin(p.a + wob2) * rr * ry;

      // profondeur fake -> taille/opacité
