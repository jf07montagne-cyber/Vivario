/* respiration.js — FINAL
   ✅ Poumons animés via classes body
...
(() => {
  ...
  function setUI(state) {
    if (state === "idle") {
      ...
      if (timer) timer.textContent = (selPreset?.value || "4-0-6").replaceAll("-", "–");
  ...
  let stageIndex = 0; // 0 inhale, 1 hold, 2 exhale, 3 hold
  let stageLeft = 0;
  let stages = [];

  let lastWholeSecond = -1; // pour décompte
  ...
  function cueCountdownIfNeeded(){
    ...
    // Décompte propre seulement sur les 3 dernières secondes
    if (sec <= 3 && sec >= 1){
      try {
        window.VivarioSound?.breathCue?.({
  ...
  async function start() {
    ...
    // optionnel : petit fond "breath_cycle"
  ...
  function pause() {
    ...
    if (!paused) {
      stageAnnounced = false; // re-cue propre
      t0 = performance.now();
      raf = requestAnimationFrame(tick);
    } else {
      cancelAnimationFrame(raf);
