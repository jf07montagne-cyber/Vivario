/* =========================================================
   Vivario — respiration.js (v23)
   DEBUG visible + test draw WebGL (Android)
   (ne touche pas au reste validé : voix/boutons/toggles/rythme)
   ========================================================= */
...
(() => {
  ...
  const DEBUG = true;          // ← laisse true pour le moment
  const DEBUG_TEST_CLEAR = true; // ← met un fond léger dans glCanvas pour prouver que ça dessine

  // badge debug dans la zone visuelle
  ...
  if (visual && DEBUG) {
    // évite double ajout si reload partiel
  ...
  if (!gl) {
    dbgSet("❌ WebGL: indisponible\n→ Sur Android: activer WebGL / essayer Chrome / désactiver “Économie d’énergie”.");
  }
  ...
  // ✅ mêmes shaders que v22 (inchangés)
  const VS = `
    attribute vec3 aPos;
    attribute vec3 aNor;
  ...
  function initGL(){
    ...
    try {
      ...
      dbgSet("❌ WebGL shader/program KO\n" + (e?.message || e));
      return;
    }
  ...
  function renderGL(info, nowMs){
    ...
    // 🔎 TEST DRAW : si tu vois un léger fond bleuté, le canvas dessine vraiment
    if (DEBUG_TEST_CLEAR) gl.clearColor(0.08, 0.16, 0.30, 0.20);
    ...
    // erreurs GL visibles via badge
    if (DEBUG) {
      const err = gl.getError();
      if (err !== gl.NO_ERROR) {
        dbgSet(dbg.textContent + `\n⚠️ gl.getError(): ${err}`);
      }
    }
  }
