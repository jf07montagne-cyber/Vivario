...
(() => {
  ...
  const FP = (gl && fragHighpOK()) ? "highp" : "mediump"; // if no highp frag, keep mediump BUT match in VS/FS
  ...
  // ===== Shader UNIQUE (organique + tubes) — compatible Android =====
  const VS = `
    ${PREC}
    attribute vec3 aPos;
    attribute vec3 aNor;
