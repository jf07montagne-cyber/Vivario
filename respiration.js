/* =========================================================
   Vivario — respiration.js (v22)
   FIX WebGL visible (matrices column-major + lookAt)
   + Dual-layer lungs mesh (outer+inner)
   + Trachea/bronchi tubes (procedural cylinders)
   + Particules 2D + souffle + voix synchronisée
...
(() => {
  ...
  /* ---------------------------
     Bind toggles (mapping IDs, sans changer le comportement)
  --------------------------- */
  ...
  /* =========================================================
     ✅ WEBGL — FIX VISIBILITÉ + DUAL-LAYER + TUBES
  ========================================================= */
  const gl = glCanvas.getContext("webgl", { antialias: true, alpha: true, premultipliedAlpha: true });
  if (!gl) console.warn("[respiration.js] WebGL non disponible.");
