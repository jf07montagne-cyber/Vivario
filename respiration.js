/* =========================================================
   Vivario — respiration.js (v24)
   FIX: shaders PRO peuvent échouer sur Android -> fallback BASIC
   + badge debug qui n’écrase plus les erreurs
...
(() => {
  ...
  let dbgSticky = "";   // message qui ne doit pas être écrasé (erreur/fallback)
  ...
  const VS_PRO = `
    ...
    void main(){
      ...
      if (uIsTube < 0.5) {
        ...
        float pulse = 0.015 * sin(uTime*2.0) + 0.025*b;
        p *= (1.0 + pulse);
        vSss = 0.15 + 0.25*b;
      }
  ...
  const FS_PRO = `
    ...
    void main(){
      ...
      vec3 baseTube  = vec3(0.70,0.92,1.00);
      ...
      float spec = pow(max(dot(N,H),0.0), 58.0) * (0.18 + 0.35*b);
  ...
  // ===== Shader BASIC (ultra compatible Android) =====
  const VS_BASIC = `
    attribute vec3 aPos;
    attribute vec3 aNor;
    ...
    void main(){
      ...
      // forme simple mais visible
      float b = clamp(uBreath, 0.0, 1.0);
      float s = 1.0 + (uIsTube < 0.5 ? (0.10 + 0.22*b) : (0.04 + 0.10*b));
      ...
      vN = normalize(mat3(uM) * normalize(p));
      vTube = uIsTube;
      vInner = uInner;
  ...
  const FS_BASIC = `
    ...
    void main(){
      ...
      vec3 lung  = mix(vec3(0.08,0.18,0.34), vec3(0.16,0.36,0.70), 0.35 + 0.65*b);
      vec3 inner = mix(vec3(0.10,0.22,0.40), vec3(0.18,0.42,0.78), 0.35 + 0.65*b);
      vec3 tube  = vec3(0.72,0.94,1.00);

      vec3 base = lung;
      float a = 0.82;

      if (vTube > 0.5) { base = tube; a = 0.55; }
      else if (vInner > 0.5) { base = inner; a = 0.62; }

      vec3 col = base * (0.25 + 0.95*ndl);
  ...
  function initGL(){
    ...
    // 1) Essai PRO
    try {
      program = createProgram(VS_PRO, FS_PRO);
      usingFallbackBasic = false;
    } catch (e) {
      const msg = (e && e.message) ? e.message : String(e);
      dbgSet("⚠️ Shader PRO KO → fallback BASIC\n" + msg, true);

      // 2) Fallback BASIC
      try {
        program = createProgram(VS_BASIC, FS_BASIC);
        usingFallbackBasic = true;
      } catch (e2) {
        const msg2 = (e2 && e2.message) ? e2.message : String(e2);
        dbgSet("❌ Shader BASIC KO\n" + msg2, true);
        program = null;
        return;
      }
    }
    ...
    loc.uTime   = gl.getUniformLocation(program, "uTime"); // peut être null en BASIC -> OK
    loc.uCam    = gl.getUniformLocation(program, "uCam");  // peut être null en BASIC -> OK
    ...
    const t2 = tubeMesh(left, 0.055, 14);
    const t3 = tubeMesh(right,0.055, 14);
    ...
    const tubes12 = merge(t1,t2);
    const tubesAll = merge(tubes12,t3);
    tubeMeshAll = createMesh(gl, tubesAll.pos, tubesAll.nor, tubesAll.idx);
  ...
  function renderGL(info, nowMs){
    ...
    // fond léger pour prouver le dessin (tu peux enlever plus tard)
    gl.clearColor(0.08, 0.16, 0.30, 0.20);
    ...
    const T  = m4(); m4Translate(T, 0, -0.10, -0.30);
    ...
    if (loc.uCam) gl.uniform3f(loc.uCam, cam[0], cam[1], cam[2]); // PRO only
    gl.uniform3f(loc.uLightDir, light[0], light[1], light[2]);
    gl.uniform1f(loc.uBreath, info.breath01);
    if (loc.uTime) gl.uniform1f(loc.uTime, nowMs * 0.001); // PRO only
