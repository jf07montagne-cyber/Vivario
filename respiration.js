/* =========================================================
   Vivario — respiration.js (v29)
   - NE TOUCHE PAS au reste validé (UI/rythme/voix/boutons/options)
   - Ajout REALISME:
     ✅ micro-relief alvéolaire procédural (bump/normal Android-safe)
     ✅ creux + AO léger (surface “vivante”)
     ✅ oxygénation plus visible (teinte + glow)
     ✅ bulles plus discrètes (ne masquent plus les détails)
   - CADRAGE/Taille: inchangé (validé)
   ========================================================= */
...
(() => {
  ...
  const VS = `
    ...
    void main(){
      ...
      if (uIsTube < 0.5) {
        ...
        // lobes plus étroits (pas de débordement)
        p.x += uSide * 0.48;

        // concavité médiale
        float medial = smoothstep(0.0, 0.55, 0.55 - abs(p.x));
        ...
        n = normalize(p);
        vSss = (0.30 + 0.56*bb) + (uInner > 0.5 ? 0.14 : 0.0);
      } else {
  ...
  // ✅ Ici est le “waouh” : alvéoles procédurales en relief (bump analytique) + AO léger
  const FS = `
    ${PREC}
    varying vec3 vN;
    varying vec3 vW;
    ...
    // Alvéoles "cellules" en 3D: sinusoïdes + phase aléatoire (analytique -> on peut dériver)
    void alveoliField(in vec3 p, out float f, out vec3 grad){
      // p: world space (déjà scaled)
      vec3 c = floor(p * 1.25);
      float j = hash3(c);                 // phase stable par "cellule"
      float k1 = 9.0, k2 = 11.0, k3 = 13.0;

      float ax = p.x*k1 + j*6.28318;
      float ay = p.y*k2 + j*4.71239;
      float az = p.z*k3 + j*5.49779;

      float sx = sin(ax), cx = cos(ax);
      float sy = sin(ay), cy = cos(ay);
      float sz = sin(az), cz = cos(az);

      // produit -> "poches" alvéolaires
      f = sx * sy * sz;
    ...
    void main(){
      ...
      // ---------- micro-relief alvéolaire ----------
      vec3 N = N0;
      float ao = 1.0;
      float alveMask = 0.0;

      if (vTube < 0.5) {
        // fréquence alvéoles (assez haute mais Android-safe)
        vec3 p = vW * 3.6;

        float f; vec3 g;
        alveoliField(p, f, g);

        // "poches" : f dans [-1..1] -> creux/bosses
        float pockets = smoothstep(0.15, 0.85, f*0.5 + 0.5);
        float ridges  = 1.0 - smoothstep(0.35, 0.70, abs(f)); // bords des alvéoles
        alveMask = clamp(0.55*pockets + 0.45*ridges, 0.0, 1.0);

        // relief plus fort quand oxy/air montent (effet “vivant”)
        float bump = (alveMask - 0.5);
        float bumpAmp = (0.28 + 0.35*air) * (0.55 + 0.45*oxy);

        // normal perturbation (direction = gradient)
        vec3 dir = normalize(g + 1e-5);
        N = normalize(N0 + dir * bump * bumpAmp * 0.22);

        // AO léger: creux + médial un peu plus sombre
        ao = 1.0 - (0.22 * (0.5 - bump)) * (0.75 + 0.25*vMed);
      ...
      vec3 ox   = vec3(0.16,0.50,0.90);
      vec3 baseLung  = mix(deox, ox, 0.22 + 0.78*oxy);
      vec3 baseInner = mix(deox*1.08, ox*1.04, 0.32 + 0.68*oxy);

      vec3 baseTubeA = vec3(0.55,0.84,1.00);
      vec3 baseTubeB = vec3(0.92,0.99,1.00);
      ...
      if (vTube > 0.5) {
        ...
        flow = smoothstep(0.12, 0.95, flow);
        vec3 tubeBase = mix(baseTubeA, baseTubeB, 0.35 + 0.65*oxy);
        base = tubeBase + flow * vec3(0.20,0.40,0.62) * (0.35 + 0.65*air);
        alpha = 0.82;
      } else if (vInner > 0.5) {
        base = baseInner;
        alpha = 0.70;
      ...
      float spec = pow(max(dot(N,H),0.0), 64.0) * (0.10 + 0.26*oxy);

      float back = max(dot(-L, N), 0.0);
      float sss = (0.10 + 0.40*vSss) * (0.45 + 0.55*back) * (0.55 + 0.45*oxy);
      vec3 sssCol = vec3(0.16,0.52,0.98) * sss;

      // ---------- renfort “alvéoles visibles” ----------
      vec3 alveTint = vec3(0.10, 0.22, 0.38) * (alveMask * 0.22);
      vec3 alveGlow = vec3(0.25, 0.60, 1.00) * (alveMask * (0.06 + 0.10*oxy) * (0.35 + 0.65*air));
      ...
      vec3 col = base * diffuse * ao
               + sssCol
               + vec3(spec)
               + rim * vec3(0.22,0.60,1.00) * (0.55 + 0.45*oxy)
               + alveGlow
               - alveTint;
  ...
  function renderGL(info, nowMs){
    ...
    oxySmooth = lerp(oxySmooth, oxyTarget, 0.07);

    const airTarget = clamp(info.air01, 0, 1);
    airSmooth = lerp(airSmooth, airTarget, 0.08);

    const dirTarget = (info.stage === "exhale") ? -1.0 : 1.0;
    flowDirSmooth = lerp(flowDirSmooth, dirTarget, 0.12);
  ...
  /* ---------------------------
     “Bulles” (2D) -> plus discrètes, juste ambiance
     (On laisse la surface 3D faire le détail)
  --------------------------- */
  ...
  function resetAlveoli(){
    alveoli = [];
    const count = 95; // ✅ moins de bulles (avant ça masquait tout)
  ...
  function drawAlveoli(info, nowMs){
    ...
    const rx = lerp(0.12, 0.14, b) * w;
    const ry = lerp(0.14, 0.17, b) * h;

    airCtx.globalCompositeOperation = "lighter";

    for(const p of alveoli){
      const wob = 0.010 * Math.sin(nowMs*0.0010 + p.seed);
      const wob2= 0.008 * Math.cos(nowMs*0.0009 + p.seed*0.7);

      const rr = p.r * (0.90 + 0.18*b);
      ...
      const size = p.size * (1.0 + 0.9*b) * depth;

      const alpha = p.glow * (0.55 + 0.55*b) * (0.55 + 0.45*depth);
