/* respiration.js — v17
   ✅ Poumons animés via classes body (CSS)
   ✅ Voix : stage + (si Coach ON) décompte plus complet
...
(() => {
  ...
  function cueCountdownIfNeeded(){
    ...
    // ✅ plus parlant si Coach ON : annonce chaque seconde
    const detailed = !!optCoach?.checked;

    // sinon, seulement les 3 dernières secondes
    if (!detailed && (sec > 3 || sec < 1)) return;
