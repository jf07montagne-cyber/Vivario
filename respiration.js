...
(() => {
  ...
  function setPhase(ph, durSec){
    ...
    // guide vocal
    if (phase === "inhale") speak("Inspire");
    else if (phase === "exhale") speak("Expire");
    else if (phase === "hold1" || phase === "hold2") speak("Bloque");
    else if (phase === "done") speak("C'est terminé. Reviens à un souffle naturel.");
  ...
  function getPhaseSequence(cfg){
    ...
    // si hold1/hold2 = 0, ils disparaissent
    return seq.length ? seq : [{ key: "inhale", dur: 4 }, { key: "exhale", dur: 6 }];
  }

  function totalCycle(cfg){
    return (cfg.inhale + cfg.hold1 + cfg.exhale + cfg.hold2) || 10;
  }

  // Progression visuelle (poumons & orbe)
  function computeScale(cfg, ph, tNorm){
    // tNorm = 0..1 dans la phase
    // inhale: scale monte
  ...
  function computeGlow(ph, tNorm){
    // un peu plus lumineux en haut d'inspire
  ...
  function updateLoop(){
    ...
    if (sessionLimit > 0) {
      ...
      const left = Math.max(0, Math.ceil(sessionLimit - elapsedSession));
      // affiche une info discrète dans le sous-texte si tu veux
      // (on garde simple: rien ici)
    ...
    // Update visuals
    const scale = computeScale(cfg, phase, tNorm);
  ...
  function pauseBreath(){
    ...
    if (paused) {
      speak("Pause");
      // stop visuals gently (keep last vars)
    } else {
      // resume loop
      phaseT0 = performance.now() - 0; // keep simple: timer resets current phase at resume
      // (Option: conserver le temps exact; mais “pro” et stable = on redémarre phase proprement)
      const cfg = getCfg();
      const seq = getPhaseSequence(cfg);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // preload voices list (certain browsers need it)
    ...
    // Changing preset/duration while running => on applique au prochain cycle (simple & stable)
    selPreset?.addEventListener("change", () => {
      if (!running || paused) return;
      // on repart sur inhale propre au prochain tick
      const cfg = getCfg();
      const seq = getPhaseSequence(cfg);
      const first = seq[0];
      setPhase(first.key, first.dur);
